 ...
 */
@WebServlet(name="modifyReservaAdulto", urlPatterns="/modifyReservaAdulto")
public class modifyReservaAdultoServlet extends HttpServlet {
		      ...
		      dispatcher.include(request, response);
		    	...
		    	GestorReservas gestorReservas = GestorReservas.getInstance();
			      ...
			      for(int i=0; i<reservasA.size(); i++) {
				  	  if(reservasA.get(i).getId() == id) {
				  		 reserva = reservasA.get(i);
				  	  }
				    }
			      if(reserva != null) {
				      ...
				      gestorReservas.modificarReservaIndividualAdultos(reserva, id);		      }
